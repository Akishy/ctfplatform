...
func (s *Storage) CreateChecker(checker *checkerDomain.Checker) error {
	...
	if _, ok := s.checkersData[checker.UUID]; ok {
		return errors.New(fmt.Sprintf("Checker-Orchestrator [%v] already exists", checker.UUID.String()))
	}
...
func (s *Storage) UpdateChecker(checker *checkerDomain.Checker) error {
	...
	defer s.mu.RUnlock()
	if _, ok := s.checkersData[checker.UUID]; !ok {
		return errors.New(fmt.Sprintf("Checker-Orchestrator [%v] not found", checker.UUID.String()))
	}
	s.checkersData[checker.UUID] = checker
	return nil
}
func (s *Storage) GetChecker(UUID uuid.UUID) (*checkerDomain.Checker, error) {
	...
	if !ok {
		return nil, errors.New(fmt.Sprintf("Checker-Orchestrator [%v] not found", UUID.String()))
	}
	return checker, nil
}
