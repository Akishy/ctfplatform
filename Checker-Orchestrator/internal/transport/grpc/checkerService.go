...
func (s *CheckerService) RegisterChecker(
	...
	if err == nil {
		s.logger.Info("Checker-Orchestrator image already exists", zap.String("code_archive", codeArchive))
		//response.Id = possibleCheckerImg.Uuid.String()
		return response, status.Error(codes.Canceled, "Checker-Orchestrator image already exists")
	}
	...
	if err := s.checkerService.CheckerImgService.Create(&checkerImg); err != nil {
		s.logger.Error("failed to create Checker-Orchestrator image", zap.Error(err))
		return nil, status.Errorf(codes.Internal, "failed to create Checker-Orchestrator image")
	}
	s.logger.Debug("Checker-Orchestrator image successfully created", zap.String("checkerImg", fmt.Sprintf("%v", checkerImg)))
	...
	if err := s.checkerService.Create(&newChecker); err != nil {
		s.logger.Error("failed to create Checker-Orchestrator", zap.Error(err))
		return nil, status.Errorf(codes.Internal, "failed to create Checker-Orchestrator")
	}
	s.logger.Debug("Checker-Orchestrator successfully created", zap.String("Checker-Orchestrator", fmt.Sprintf("%v", newChecker)))
...
func (s *CheckerService) PingChecker(
	...
	if err != nil {
		s.logger.Error("failed to parse Checker-Orchestrator uuid", zap.Error(err))
...
func (s *CheckerService) CreateVulnService(_ context.Context, req *proto.CreateVulnServiceRequest) (*proto.CreateVulnServiceResponse, error) {
	...
	if exists := s.checkerService.Exists(checkerUUID); !exists {
		s.logger.Error("failed to find Checker-Orchestrator uuid", zap.String("req id", req.GetServiceId()))
		return nil, status.Errorf(codes.InvalidArgument, "failed to find Checker-Orchestrator uuid")
	}
