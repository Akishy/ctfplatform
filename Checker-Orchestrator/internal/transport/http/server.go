...
func New(port int, vulnServiceService *vulnServiceService.Service, checkerService *checkerService.Service, logger *zap.Logger) *ServiceServer {
	...
	checkerRouter := router.PathPrefix("/Checker-Orchestrator").Subrouter()
	...
	router.PathPrefix("/Checker-Orchestrator").Handler(checkerRouter)
...
func (s *ServiceServer) SubscribeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.logger.Error("failed to set Checker-Orchestrator address", zap.Error(err))
		_, _ = w.Write([]byte("failed to set Checker-Orchestrator address"))
		respondErr := jsonUtils.RespondWith500(w)
		if respondErr != nil {
			s.logger.Error("failed to write response", zap.Error(err))
			return
